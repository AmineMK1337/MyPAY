...
import com.mypay.dto.MonthlyForecastResponse;
...
public class PaymentController {
    ...
    public ResponseEntity<ApiResponse> getPaymentHistory(@RequestHeader("Authorization") String token) {
        try {
            String userId = getUserIdFromToken(token);
            List<Payment> payments = paymentService.getPaymentsByUser(userId);
            return ResponseEntity.ok(new ApiResponse(true, "Payment history fetched", payments));
        } catch (Exception e) {
            return ResponseEntity.status(400).body(new ApiResponse(false, e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse> getMonthlyForecast(@RequestHeader("Authorization") String token) {
        try {
            String userId = getUserIdFromToken(token);
            double total = paymentService.getMonthlyForecastTotal(userId);
            int activeContracts = paymentService.getActiveContracts(userId).size();
            MonthlyForecastResponse response = new MonthlyForecastResponse(total, activeContracts);
            return ResponseEntity.ok(new ApiResponse(true, "Monthly forecast computed", response));
        } catch (Exception e) {
            return ResponseEntity.status(400).body(new ApiResponse(false, e.getMessage()));
        }
    }
